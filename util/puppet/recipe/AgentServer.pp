class MadeiraCloud::WebInstall{
	...
	$yumpackages = [ "mercurial", "php", "php-xml", "php-mysql", "php-mcrypt", "php-devel", "php-pear", "gmp-devel", "mysql", "mysql-server", "nginx", "gcc", "make", "gcc-c++", "uuid-devel", "libuuid-devel", "git","expect" ]
	...
	exec { "pear install":
	    command => "echo \"isInstall=`pecl list| grep mongo`;if [[ \\\$isInstall != *mongo* ]];then echo 'pear channel-discover pear.zero.mq; pecl install mongo; pecl install zero.mq/zmq-beta'| sh;echo \\\"extension=mongo.so\\\">>/etc/php.ini;fi\" | sh",
	...
	exec{"mongodb_start":
		cwd=>$root,
		command =>  "echo \"isExists=`pgrep mongo`; if [ \\\"\\\$isExists\\\"==\\\"\\\" ]; then configDB=\\\"`curl -s --data \\\"method=getMongoConfig\\\" https://api.madeiracloud.com/monitorconfig/`\\\"; $root/mongodb-linux-x86_64-2.0.5/bin/mongos -configdb \\\$configDB -port 30000 -logpath $MADEIRA/log/mongos.log -logappend -fork; fi\" | sh",
		path => "/usr/bin:/usr/sbin:/bin"
	...
	exec { "MadeiraClone":
		cwd=>$root,
		command =>  "echo \"if [ ! -d $MADEIRA/site ]; then echo 'set timeout 600; spawn hg clone ssh://$HG//opt/source/mainline/Madeira $MADEIRA ;expect \\\"(yes/no)?\\\" {send \\\"yes\\r\\\"; expect \\\"*password:\\\"; send \\\"InstantCloud123\\!\\@\\#\\r\\\"} \\\"*password:\\\" { send \\\"InstantCloud123\\!\\@\\#\\r\\\"};expect eof'| expect; fi\" | sh",
		path => "/usr/bin:/usr/sbin:/bin",
	...
	exec { "MadeiraPull":
		cwd=>$MADEIRA,
		command =>  "echo \"echo 'set timeout 600; spawn hg pull ssh://$HG//opt/source/mainline/Madeira;expect \\\"*password:\\\";send \\\"InstantCloud123\\!\\@\\#\\r\\\";expect eof'| expect\" | sh",
		path => "/usr/bin:/usr/sbin:/bin",
	...
	exec { "VulcanClone":
		cwd=>$SOURCE,
		command =>  "echo \"if [ ! -d $SOURCE/Vulcan/Util ]; then echo 'set timeout 600; spawn hg clone ssh://$HG//opt/source/mainline/Vulcan $SOURCE/Vulcan ;expect \\\"*password:\\\";send \\\"InstantCloud123\\!\\@\\#\\r\\\";expect eof'| expect; cp -rf $SOURCE/Vulcan/Util/script/* $SITE/download/setup/; fi\" | sh",		
	...
	exec { "VulcanPull":
		cwd=>"$SOURCE/Vulcan",
		command =>  "echo \"echo 'set timeout 600; spawn hg pull ssh://$HG//opt/source/mainline/Vulcan;expect \\\"*password:\\\";send \\\"InstantCloud123\\!\\@\\#\\r\\\";expect eof'| expect; cp -rf $SOURCE/Vulcan/Util/script/* $SITE/download/setup/\" | sh",
		path => "/usr/bin:/usr/sbin:/bin",
		subscribe => File["/etc/madeiracloud.version"],
		refreshonly => true,
		timeout => "-1"
	...
	exec { "IDEClone":
		cwd=>"$SITE/ide/",
		command =>  "echo \"if [ ! -d $SITE/ide/src ]; then  echo 'set timeout 600; spawn scp root@211.98.26.6:/opt/release/beta/pugna.7z $SITE/ide/ ;expect \\\"*password:\\\";send \\\"InstantCloud123\\!\\@\\#\\r\\\";expect eof' | expect; 7za x $SITE/ide/pugna.7z; rm -f $SITE/ide/pugna.7z; fi\" | sh",
		path => "/usr/bin:/usr/sbin:/bin",
		timeout => "-1"	
	}
	...
	exec { "IDEPull":
		cwd=>"$SITE",
		command =>  "echo \"rm -rf $SITE/ide/; mkdir -p $SITE/ide; echo 'set timeout 600; spawn scp root@211.98.26.6:/opt/release/beta/pugna.7z $SITE/ide/ ;expect \\\"*password:\\\";send \\\"InstantCloud123\\!\\@\\#\\r\\\";expect eof' | expect; cd $SITE/ide/; 7za x $SITE/ide/pugna.7z; rm -f $SITE/ide/pugna.7z;\" | sh",
		path => "/usr/bin:/usr/sbin:/bin",
		subscribe => File["/etc/madeiracloud.version"],
		refreshonly => true,
		timeout => "-1"
